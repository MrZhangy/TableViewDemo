...
-(id)initWithFrame:(CGRect)frame titles:(NSArray*)titleArray viewArray:(NSArray *)vcArray
{
...
-(void)setMenuBar
{
    _headerScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 40)];
...
-(void)setSelectedButton:(UIButton *)button
{
    ...
    [_collectionView setContentOffset:CGPointMake((button.tag - 100)* CGRectGetWidth(self.frame), 0) animated:YES];
}

-(void)setCollectionView
{
    CGRect frame = CGRectMake(0, CGRectGetMaxY(_headerScrollView.frame), self.frame.size.width, self.frame.size.height - CGRectGetHeight(_headerScrollView.frame));
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /*
    UIViewController * viewvc = self.viewArray[indexPath.item];
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger index = (NSInteger)floor((point.x / self.frame.size.width));
    ...
    if (self.delegate != nil && [self.delegate conformsToProtocol:NSProtocolFromString(@"MenuViewControllerDelegate")]) {
        [self.delegate menuView:self currentPageChanged:index];
    }
